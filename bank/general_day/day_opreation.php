...
echo'<table width="100%" border="2" cellpadding="0" cellspacing="0">
    ...
    <tr>
      ...
      <td dir="LTR" width="210">(الشهرالسابق/المدة    السابقة)</td>
      <td dir="LTR" width="220">الشهرالجاري    /المدة الجارية </td>
      <td dir="LTR" width="634">الشهرالجاري    /المدة الجارية </td>
      <td dir="LTR" width="229">(الشهرالسابق/المدة    السابقة  )</td>
...
echo $num_account;
$sqlstatment3='SELECT `year`, `month`, `term`, `type_account`, `accounts`, sum(`values`) FROM `later` WHERE `year`="'.$year.'" And `month`<="'.$monthinsert.'" AND `day`="'.$day.'" AND `type_account`="'.$i.'" AND`accounts`="'.$num_account.'" AND `term`=2 ';
...
$sqlstatment5='SELECT `num_ts`, `num_s`, `year`, `month`, `day`, `data`, `term`, `type_account`, `accounts`, `resources`, `uses`, sum(`val`), `num_coll` FROM `public` WHERE  `year`="'.$year.'" AND `day`="'.$day.'" And `month`="'.$monthinsert.'"  AND `type_account`="'.$i.'" AND`accounts` ="'.$num_account.'" AND `term`=2 ';
...
$sqlstatment4='SELECT `year`, `month`, `term`, `type_account`, `accounts`, sum(`values`) FROM `later` WHERE `year`="'.$year.'" AND `day`="'.$day.'" And `month`<="'.$monthinsert.'" AND  `type_account`="'.$i.'" AND`accounts`="'.$num_account.'" AND  `term`=1 ';
...
$sqlstatment6='SELECT  `year`, `month`, `term`, `type_account`, `accounts`, sum(`val`) FROM `public` WHERE  `year`="'.$year.'" And `month`="'.$monthinsert.'" AND `day`="'.$day.'"  AND `type_account`="'.$i.'" AND`accounts` ="'.$num_account.'" AND `term`=1';
...
echo'
 <td dir="RTL">'.$sumgmcur.'</td>
	...
	$query = mysqli_query($sss, "INSERT INTO `later`(`year`, `month`,`day`, `term`, `type_account`, `accounts`, `values`) 
VALUES('$year','$monthinsert','$day,'1','$i','$num_account','$sumlagm')") or die (mysqli_error($sss)); 
$query = mysqli_query($sss, "INSERT INTO `later`(`year`, `month`,`day`, `term`, `type_account`, `accounts`, `values`) 
        VALUES('$year','$monthinsert','$day,'2','$i','$num_account','$sumlagd')") or die (mysqli_error($sss)); 
